...
export class SouscriptionContratSignal {
  ...
  readonly formSouscription = form(this.model, (path) => {
    ...
    validate(path.clientId, ({ value }) =>
      value() != null && !Number.isInteger(value())
        ? { kind: 'entier', message: 'Le n° client doit etre entier .' }
        : null,
    );
    ...
    validate(path.optionsChoisies, ({ value }) =>
      value().length === 0 ? { kind: 'aucuneOption', message: 'Choisir une options.' } : null,
    );
  });
  ...
  readonly prime = computed(() => {
    ...
    if (!m.typeDeContrat || m.ageClient == null) {
      // on vérifie avant le calcule
      return 0;
    }
